    ...
    <div className="min-h-screen bg-[#0f172a]">
      ...
      <main className="lg:ml-64 p-6 lg:p-8">
        <div className="max-w-6xl mx-auto space-y-6">
          <div>
            <h1 className="text-2xl font-bold text-white">Activos & Inversiones</h1>
            <p className="text-[#94a3b8]">Patrimonio y portafolio de inversiones</p>
          </div>

          <DollarBanner onCotizacionChange={setCotizacion} onMonedaChange={setMoneda} />

          {/* Grand Total */}
          <div className="bg-gradient-to-r from-[#1e293b] to-[#0f172a] rounded-xl border border-[#334155] p-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <p className="text-sm text-[#94a3b8]">Total Patrimonio</p>
                <p className="text-3xl font-bold text-white mt-1">{fmt(grandTotal)}</p>
              </div>
              {dailyDiff !== 0 && (
          ...
          {chartData.length > 1 && (
            <div className="bg-[#1e293b] rounded-xl border border-[#334155] p-6">
              <h2 className="text-lg font-semibold text-white mb-4">Evolución del Patrimonio</h2>
              <div className="h-64">
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-[#334155]">
              ...
              <button onClick={openAddActivo} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-colors">
            ...
            {activosList.length > 0 ? (
              <div className="divide-y divide-[#334155]">
                {activosList.map((a) => (
                  <div key={a.id} className="flex items-center justify-between px-4 py-3 hover:bg-[#334155]/30 transition-colors">
                    <div>
                      <span className="font-medium text-white">{a.entidad}</span>
                      <span className="text-[#94a3b8] ml-2">- {a.descripcion}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-medium text-white">{fmt(Number(a.monto))}</span>
                      <button onClick={() => openEditActivo(a)} className="text-[#64748b] hover:text-blue-400"><Pencil size={16} /></button>
                      <button onClick={() => deleteActivo(a.id)} className="text-[#64748b] hover:text-red-400"><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="px-4 py-4 text-[#64748b] text-sm">Sin activos registrados</p>
            )}
            <div className="px-4 py-3 border-t border-[#475569] bg-[#0f172a]/50">
              <div className="flex justify-between">
                <span className="font-bold text-white">Subtotal Activos</span>
                <span className="font-bold text-white">{fmt(activosList.reduce((s, a) => s + Number(a.monto), 0))}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-[#334155]">
              <h2 className="text-lg font-semibold text-white">Inversiones (Entidades)</h2>
              <button
                onClick={() => {
                  setEditActivo(null);
                  setFEntidad(ENTIDADES[0]);
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-purple-500 hover:bg-purple-600 text-white text-sm font-medium transition-colors"
              >
                <Plus size={16} /> Agregar
              </button>
            </div>
            {inversionesList.length > 0 ? (
              ...
              <p className="px-4 py-4 text-[#64748b] text-sm">Sin inversiones registradas</p>
            )}
            <div className="px-4 py-3 border-t border-[#475569] bg-[#0f172a]/50">
              <div className="flex justify-between">
                <span className="font-bold text-white">Subtotal Inversiones</span>
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-[#334155]">
              <h2 className="text-lg font-semibold text-white">Cocos Capital - Inversiones</h2>
            ...
            {TIPOS_COCOS.map((tipo) => {
              ...
              return (
                <div key={tipo}>
                  <div className="px-4 py-2 bg-[#334155]/30">
                    <span className="text-sm font-medium text-yellow-400">{tipo}</span>
                  </div>
                  <div className="divide-y divide-[#334155]">
                    {items.map((i) => (
                      <div key={i.id} className="flex items-center justify-between px-4 py-3 hover:bg-[#334155]/30 transition-colors">
                        <span className="text-[#94a3b8]">{i.descripcion || tipo}</span>
                        <div className="flex items-center gap-3">
                          <span className="font-medium text-white">{fmt(Number(i.monto))}</span>
                          <button onClick={() => openEditInv(i)} className="text-[#64748b] hover:text-blue-400"><Pencil size={16} /></button>
                          <button onClick={() => deleteInv(i.id)} className="text-[#64748b] hover:text-red-400"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {inversiones.length === 0 && (
              <p className="px-4 py-4 text-[#64748b] text-sm">Sin inversiones en Cocos Capital</p>
            )}
            <div className="px-4 py-3 border-t border-[#475569] bg-[#0f172a]/50">
              <div className="flex justify-between">
                <span className="font-bold text-white">Subtotal Cocos Capital</span>
          ...
          <Modal isOpen={modalActivo} onClose={() => setModalActivo(false)} title={editActivo ? "Editar" : "Agregar Activo/Inversión"}>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#94a3b8] mb-1">Entidad</label>
                <select value={fEntidad} onChange={(e) => setFEntidad(e.target.value)} className="w-full bg-[#334155] border border-[#475569] rounded-lg px-3 py-2 text-white">
                  {ENTIDADES.map((e) => <option key={e} value={e}>{e}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#94a3b8] mb-1">Tipo</label>
                <select value={fTipo} onChange={(e) => setFTipo(e.target.value)} className="w-full bg-[#334155] border border-[#475569] rounded-lg px-3 py-2 text-white">
                  {TIPOS.map((t) => <option key={t} value={t}>{t === "activo" ? "Activo" : "Inversión"}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#94a3b8] mb-1">Descripción</label>
                <input type="text" value={fDesc} onChange={(e) => setFDesc(e.target.value)} placeholder="Ej: Caja de Ahorro, FIMA..." className="w-full bg-[#334155] border border-[#475569] rounded-lg px-3 py-2 text-white" />
              </div>
              <div>
              ...
              <button onClick={saveActivo} className="w-full py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-medium transition-colors">
          ...
          <Modal isOpen={modalInv} onClose={() => setModalInv(false)} title={editInv ? "Editar Inversión Cocos" : "Agregar Inversión Cocos"}>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#94a3b8] mb-1">Tipo</label>
                <select value={fInvTipo} onChange={(e) => setFInvTipo(e.target.value)} className="w-full bg-[#334155] border border-[#475569] rounded-lg px-3 py-2 text-white">
                  {TIPOS_COCOS.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#94a3b8] mb-1">Descripción</label>
                <input type="text" value={fInvDesc} onChange={(e) => setFInvDesc(e.target.value)} placeholder="Descripción..." className="w-full bg-[#334155] border border-[#475569] rounded-lg px-3 py-2 text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#94a3b8] mb-1">Monto</label>
                <input type="number" value={fInvMonto} onChange={(e) => setFInvMonto(e.target.value)} placeholder="0.00" className="w-full bg-[#334155] border border-[#475569] rounded-lg px-3 py-2 text-white" />
              </div>
